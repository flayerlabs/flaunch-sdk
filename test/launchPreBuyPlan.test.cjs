...
/** Cost model: 1e27 coins (1%) cost 1000 units before slippage; slippage adds bps on top. */
...
test("route A (Base standard): three pinned fee reads, fee/expected/max, bound calldata", async () => {
  ...
  assert.equal(plan.zapFamily, "legacy");
  assert.equal(lower(plan.launch.to), lower(FlaunchZapAddress[base.id]));

  const feeReads = reads(drift, "calculateFee");
  assert.equal(feeReads.length, 3);
  for (const read of feeReads) {
    assert.equal(lower(read.address), lower(FlaunchZapAddress[base.id]));
    assert.deepEqual(read.options, { block: BLOCK });
  }
  assert.deepEqual(
    feeReads.map(({ args }) => [args._premineAmount, args._slippage]),
    [[0n, 0n], [premine, 0n], [premine, 50n]]
  );
  assert.equal(plan.fee.amount, FEE);
  assert.deepEqual(plan.fee.asset, { chainId: base.id, address: zeroAddress, decimals: 18 });
  assert.equal(plan.payment.expected, 1000n);
  assert.equal(plan.payment.max, 1005n);
  assert.equal(plan.value, FEE + 1005n);
  ...
  const decoded = decodeFunctionData({ abi: FlaunchZapV1_1_6Abi, data: plan.launch.data });
...
test("route B (Robinhood dynamic split): manager overload, fee read with premine zeroed", async () => {
  ...
  assert.equal(plan.payment.expected, 10_000n);
  assert.equal(plan.payment.max, 10_100n);
...
test("route C ERC20 pairing (6 dp): payment asset, zap approval, value is the fee only", async () => {
  ...
  assert.equal(plan.payment.expected, 1000n);
  assert.equal(plan.payment.max, 1005n);
  assert.equal(plan.maxPremineCost, 1005n);
  ...
  assert.equal(decoded.args[1], zeroAddress);
  assert.equal(decoded.args[2], 1005n);
});

test("route C ERC20: standing allowance drops the approval; approvalAllowance sizes it", async () => {
  const covered = new ReadFlaunchSDK(base.id, fakeDrift({ allowance: 1005n }));
  ...
  const sized = new ReadFlaunchSDK(base.id, fakeDrift());
...
test("route C native pairings: raw ETH skips the registry, flETH resolves to native, unapproved is rejected", async () => {
  const raw = fakeDrift();
  const { plan: rawPlan } = await new ReadFlaunchSDK(base.id, raw).planLaunchPreBuy({
    route: "pairedToken",
  ...
  assert.equal(rawPlan.value, FEE + 1005n);
  assert.equal(rawPlan.payment.expected, 1000n);
  assert.equal(rawPlan.payment.max, 1005n);
  assert.equal(rawPlan.maxPremineCost, 1005n, "zap's pairedPremineCost passes through");
  ...
  const { plan: flethPlan } = await new ReadFlaunchSDK(base.id, fleth).planLaunchPreBuy({
    route: "pairedToken",
    params: pairedParams,
    preBuyBps: 100,
    slippageBps: 50,
  ...
  assert.equal(flethPlan.value, FEE + 1005n);
...
test("static rejections return before any RPC; no signer yields SENDER_REQUIRED", async () => {
  ...
  const noSender = await new ReadFlaunchSDK(base.id, readOnly).planLaunchPreBuy({
    route: "standard",
    params: standardParams,
    preBuyBps: 100,
    slippageBps: 50,
  ...
  const explicit = await new ReadFlaunchSDK(base.id, readOnly).planLaunchPreBuy({
    route: "standard",
    params: standardParams,
    preBuyBps: 100,
    slippageBps: 50,
...
test("funding shortfall is reported at plan time and blocks execution before any write", async () => {
  ...
  const sdk = new ReadWriteFlaunchSDK(base.id, drift);
  const { plan } = await sdk.planLaunchPreBuy({
    route: "standard",
    params: standardParams,
    preBuyBps: 100,
  ...
  const pairedSdk = new ReadWriteFlaunchSDK(base.id, erc20);
...
async function plannedOnBase(overrides = {}) {
...
test("any edit, expiry, signer or chain change requires a fresh quote and sends nothing", async () => {
  ...
  await expectRequote(plan, "CHAIN_MISMATCH", new ReadWriteFlaunchSDK(robinhood.id, fakeDrift()));
...
test("a fresh quote above the plan's value is PRICE_MOVED; a higher fee is FEE_CHANGED", async () => {
  const moved = await plannedOnBase();
  ...
  // price within the slippage headroom still executes
  const within = await plannedOnBase();
...
test("simulation revert requires a fresh quote; verify(simulate) reports ethSpent", async () => {
  const reverted = await plannedOnBase({ simulate: new Error("InsufficientPreminePayment") });
...
test("route A execute writes the plan's exact arguments and value, once", async () => {
  const { drift, sdk, plan } = await plannedOnBase();
  ...
  const [write] = all;
  assert.equal(lower(write.address), lower(FlaunchZapAddress[base.id]));
  ...
  assert.deepEqual(write.args._treasuryManagerParams, decoded.args[5]);
  ...
  const none = await plannedOnBase();
...
test("route C ERC20 execute: approval first and awaited, then flaunch with maxPremineCost and fee-only value", async () => {
  ...
  assert.deepEqual(all[0].args, { spender: FlaunchZapV1_3Address[base.id], amount: 1005n });
  assert.equal(all[1].fn, "flaunch");
  assert.equal(lower(all[1].address), lower(FlaunchZapV1_3Address[base.id]));
  assert.equal(all[1].args._maxPremineCost, 1005n);
  ...
  const failedSdk = new ReadWriteFlaunchSDK(base.id, failed);
...
test("flaunchWithPreBuy throws a typed error for unsupported launches", async () => {
  const sdk = new ReadWriteFlaunchSDK(base.id, fakeDrift());
  await assert.rejects(
      ...
      async request({ method, params }) {
        ...
        const answer = (data) => {
          ...
          if (decoded.functionName === "calculateFee") {
            return encodeAbiParameters([{ type: "uint256" }], [FEE + premineCost(decoded.args[0], decoded.args[1])]);
          }
          if (decoded.functionName === "flaunch") {
            return encodeAbiParameters(
              [{ type: "address" }, { type: "uint256" }, { type: "address" }],
              [MEMECOIN, 1n, zeroAddress]
            );
          }
